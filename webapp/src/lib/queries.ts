...
export function useBootstrapData() {
  ...
  const matchCountQuery = useQuery({
    ...
    queryFn: () => getJson<{ total: number }>('/ocrux/match/count-match-issues'),
    refetchInterval: 60_000,
  })

  const autoMatchQuery = useQuery({
    queryKey: queryKeys.autoMatches,
    queryFn: () => getJson<{ total: number }>('/ocrux/match/count-new-auto-matches'),
    refetchInterval: 60_000,
  })
  ...
  useEffect(() => {
    ...
    setCounts({
      totalMatchIssues: matchCountQuery.data.total,
      totalAutoMatches: autoMatchQuery.data.total,
    })
