...
import mixin from '@shared/utils/generalMixin.ts';
import fs from 'node:fs';
import fhirPkg from 'fhir';
import decisionRulesTest from '../../../config/decisionRulesTest.json';
import decisionRulesProd from '../../../config/decisionRules.json';
    ...
    Promise.all(promises).then(() => {
      async.eachSeries(files, (file, nxtFile) => {
        fs.readFile(`${__dirname}/../../../../resources/Relationships/${file}`, (err, data) => {
          ...
          if(detailsExt) {
            const decisionRules = process.env.NODE_ENV === 'test' ? decisionRulesTest : decisionRulesProd;
