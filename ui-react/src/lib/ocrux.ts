...
export async function fetchPatients(query = '') {
  return getJson<FhirBundle<FhirPatient>>(`/ocrux/fhir/Patient${query}`)
}
...
export async function fetchCSVReport(id: string) {
  const response = await fetch(`/ocrux/csv/getCSVReport/${encodeURIComponent(id)}`, {
    credentials: 'include',
  })
...
export async function authenticate(username: string, password: string) {
  return getJson<{ token: string | null; userID: string | null; username: string; role: string | null }>(
